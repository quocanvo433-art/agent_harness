...
### 3. Quy trình Quản lý Vòng đời Tác tử (Strict Subagent Lifecycle Management)
Lead Architect làm nhiệm vụ chỉ huy cao nhất, có nghĩa vụ quản lý và kiểm soát toàn bộ vòng đời của các subagent được triệu hồi trong phiên làm việc:
*   **Triệu hồi có mục đích:** Chỉ khởi chạy subagent (`invoke_subagent`) khi có tác vụ bóc tách cụ thể (ví dụ: cần code, cần audit tĩnh, hoặc cần test/đánh giá lỗi).
*   **Bắt buộc Đóng/Hủy tác tử (Mandatory Shutdown):** Ngay sau khi subagent trả về kết quả (thành công hoặc thất bại) hoặc khi tác vụ tương ứng của subagent kết thúc, Lead Architect **bắt buộc phải sử dụng công cụ `manage_subagents` với hành động `kill`** và truyền vào đúng `conversationId` của subagent để giải phóng tài nguyên.
*   **Cấm bỏ quên Zombie Agents:** Tuyệt đối không được kết thúc phiên làm việc (merge code hoặc submit sổ cái) mà vẫn còn subagent ở trạng thái hoạt động (`active`). Trước khi kết thúc phiên, bắt buộc phải chạy lệnh `manage_subagents` với hành động `list` để kiểm tra chéo và thực hiện đóng toàn bộ (`kill_all` hoặc `kill` từng ID) nếu còn rác tiến trình tác tử.
