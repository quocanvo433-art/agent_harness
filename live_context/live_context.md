# 📖 LIVE CONTEXT - NGỮ CẢNH PHIÊN LÀM VIỆC DỰ ÁN (APEX Swarm v3.0)

> ⚡ **CẢNH BÁO CHO AI AGENT (ANTIGRAVITY 2.0 NATIVE):** File này được sinh tự động bởi `live_context_loader.py`.
> **Thời gian cập nhật:** 2026-06-24 12:00:10
> **File đang thao tác:** `dashboard/dashboard_ui/ContentComposer.jsx`
> **Tài liệu đặc tả chính:** [Spec 08 - facepost_08_content_engine.md](file:///C:/AI_Facepost/specs/facepost_08_content_engine.md)
...
### 🌐 [SHARED TYPES CONSTITUTION] Spec 00 - facepost_00_shared_types.md

# 📋 Spec 00 — Shared Types & Architecture Decisions

**Version:** 2.2.0 (Post-Audit Security Upgrade)  
**Status:** ACTIVE  
**Updated:** 2026-06-18  

Tài liệu này đặc tả chi tiết các hợp đồng dữ liệu dùng chung (Shared Types), giao thức truyền tải, cấu trúc cơ sở dữ liệu và các quy định an toàn hệ thống (Security Decisions) áp dụng thống nhất giữa **Chrome Extension** (Client) và **Dashboard Server** (Server).

---

## 1. Hướng Dẫn Kiến Trúc & Bảo Mật Hệ Thống (Design Anchors)

Mọi cấu phần phần mềm trong hệ thống phải tuân thủ nghiêm ngặt các nguyên tắc kiến trúc và giải pháp bảo mật dưới đây để ngăn ngừa các tấn công khai thác và đảm bảo tính ổn định luồng chạy.

### 1.1 Chống Tấn Công Phát Lại (Replay Attack Prevention)
* **Vấn đề:** Trong luồng bắt tay WebSocket, nếu kẻ tấn công chặn được bản tin xác thực đầu tiên (`HELLO`), chúng có thể gửi lại bản tin đó để giả mạo một kết nối hợp lệ trong tương lai.
* **Giải pháp:** 
  1. **Nonce tracking:** Hệ thống Dashboard Server bắt buộc phải duy trì một tập hợp nonce đã nhận (`nonce Set`) trong bộ nhớ tạm (In-memory Set hoặc Redis cache) với thời gian sống **TTL = 30 giây** (trùng khớp với độ lệch thời gian cho phép của timestamp).
  2. **Kiểm tra trùng lặp:** Khi nhận bản tin `HELLO`, Server thực hiện kiểm tra:
     - Nếu `nonce` đã tồn tại trong `nonce Set`, ngắt kết nối lập tức.
     - Nếu `nonce` chưa tồn tại và `timestamp` nằm trong phạm vi cho phép ($\pm30$ giây so với giờ Server), thêm `nonce` vào Set và thiết lập tự động xóa sau 30 giây.

### 1.2 Chống Prototype Pollution & Ép Kiểu Đầu Vào (Data Sanitization)
* **Vấn đề:** Payload WebSocket hoặc dữ liệu JSON lồng nhau khi parse tự động trong JavaScript có thể chứa các thuộc tính nguy hiểm như `__proto__`, `constructor`, hoặc `prototype`, dẫn đến sửa đổi Object gốc của Node.js, gây lỗi logic hoặc thực thi mã từ xa (RCE).
* **Giải pháp:**
  1. **Schema Validation:** Toàn bộ bản tin WebSocket hoặc các API đầu vào phải được xác thực thông qua bộ parser nghiêm ngặt (ví dụ: thư viện **Ajv**) với cấu hình cấm thuộc tính thừa (`additionalProperties: false`).
  2. **Object Nguyên Bản (Clean Maps):** Khi lưu trữ các cấu trúc ánh xạ động từ client (như `mediaBuffer` hoặc `activeSessions`), bắt buộc sử dụng `Object.create(null)` thay vì `{}` để triệt tiêu việc kế thừa các thuộc tính và phương thức từ `Object.prototype`.
  3. **Type Check:** Thực hiện kiểm tra kiểu dữ liệu tường minh trước khi xử lý chuỗi hoặc truy vấn (ví dụ: `typeof payload.accountId === "string"`), tuyệt đối không tin tưởng cơ chế tự ép kiểu (Type Coercion).

### 1.3 Giới Hạn RangeError & OOM trong Truyền Tải File (Media Attachment Safeguards)
* **Vấn đề:** Khởi tạo mảng động qua tham số client gửi lên (ví dụ: `new Array(totalChunks)`) mà không kiểm tra giới hạn có thể dẫn đến lỗi `RangeError: Invalid array length` hoặc cạn kiệt bộ nhớ hệ thống (Out Of Memory - OOM).
* **Giải pháp:**
  1. **Ràng buộc toán học:** Tham số `totalChunks` phải là một số nguyên dương hợp lệ (`Number.isInteger(totalChunks) && totalChunks > 0`).
  2. **Giới hạn cận trên:** Giới hạn tối đa của `totalChunks` được tính toán dựa trên dung lượng file tối đa (50MB) và kích thước chunk tối thiểu (500KB):
     $$\text{totalChunks} \le \frac{50 \times 1024 \times 1024}{500 \times 1024} = 103$$
     Nếu `totalChunks > 103` hoặc kích thước vượt quá cấu hình, Server/Extension phải từ chối xử lý bản tin và ném lỗi `ERR-MEDIA-LIMIT`.

### 1.4 Giải Mã Base64 Không Chặn Event Loop (Non-Blocking Decode)
* **Vấn đề:** Việc giải mã các file media lớn (ví dụ video 50MB) bằng vòng lặp đồng bộ `atob` và gán byte thủ công trên JS main thread sẽ gây đóng băng toàn bộ hoạt động của Extension trong vài giây, làm đứt kết nối WebSocket hoặc bị trình duyệt tự động chấm dứt tiến trình.
* **Giải pháp:**
  Tận dụng luồng xử lý nền (C++ background threads) của trình duyệt bằng cách giải mã bất đồng bộ thông qua đối tượng native `Response` và `Blob`:
  ```javascript
  const fullBase64 = chunks.join('');
  const response = await fetch(`data:${mimeType};base64,${fullBase64}`);
  const blob = await response.blob();
  const arrayBuffer = await blob.arrayBuffer();
  const fileBytes = new Uint8Array(arrayBuffer);
  ```

### 1.5 Chuẩn Hóa Tọa Độ Tuyệt Đối DOM (Absolute Coordinate Fingerprint)
* **Vấn đề:** Việc băm định danh phần tử (Fingerprint) dựa trên `getBoundingClientRect()` của phần tử DOM sẽ bị thay đổi khi cuộn trang (do trả về tọa độ tương đối so với khung nhìn viewport), làm sai lệch kết quả click của AI Agent.
* **Giải pháp:**
  Chuẩn hóa tọa độ tương đối của phần tử thành tọa độ tuyệt đối so với tài liệu (Document-relative coordinates):
  ```javascript
  const rect = el.getBoundingClientRect();
  const absoluteX = Math.round(rect.x + window.scrollX);
  const absoluteY = Math.round(rect.y + window.scrollY);
  ```

---

## 2. Đặc Tả JSDoc Types & Hệ Thống Kiểu Dữ Liệu

Dưới đây là các định nghĩa kiểu dữ liệu chuẩn (Shared Typedefs) dùng chung cho toàn hệ thống:

```javascript
/**
 * @typedef {Object} DOMElement
 * @property {string} elementId - Chuỗi băm 8 ký tự hex (FNV-1a 32-bit) định danh duy nhất phần tử
 * @property {string} tagName - Tên thẻ HTML (viết hoa, ví dụ: "BUTTON", "INPUT")
 * @property {string} [role] - Thuộc tính ARIA role nếu có
 * @property {string} [ariaLabel] - Giá trị của thuộc tính aria-label
 * @property {string} [dataTestId] - Giá trị của data-testid
 * @property {string} [innerText] - Văn bản hiển thị của phần tử (giới hạn tối đa 100 ký tự)
 * @property {number} absoluteX - Tọa độ X tuyệt đối (rect.x + window.scrollX)
 * @property {number} absoluteY - Tọa độ Y tuyệt đối (rect.y + window.scrollY)
 * @property {number} width - Chiều rộng phần tử
 * @property {number} height - Chiều cao phần tử
 * @property {boolean} isVisible - Trạng thái hiển thị thực tế trên màn hình
 */

/**
 * @typedef {Object} DOMSnapshot
 * @property {string} url - URL hiện tại của trang web
 * @property {number} timestamp - Thời điểm quét snapshot (Epoch Milliseconds)
 * @property {DOMElement[]} elements - Danh sách các phần tử tương tác được rút trích và nén
 * @property {string} [visualEvidence] - Chuỗi Base64 ảnh chụp màn hình (chỉ đính kèm ở Tier 4)
 */

/**
 * @typedef {Object} MediaMetadata
 * @property {string} mediaId - Định danh duy nhất của tệp đính kèm
 * @property {string} mimeType - Kiểu MIME của tệp (ví dụ: "image/jpeg", "video/mp4")
 * @property {number} size - Kích thước tệp (bytes)
 */

/**
 * @typedef {Object} SessionContext
 * @property {string} accountId - Định danh tài khoản Facebook đang chạy
 * @property {string} campaignId - Định danh chiến dịch hiện hành
 * @property {MediaMetadata[]} mediaMetadata - Siêu dữ liệu của các file đính kèm (không chứa Base64 nhị phân)
 * @property {string[]} visitedGroupIds - Danh sách ID nhóm đã đăng bài thành công trong phiên này
 * @property {number} dailyPostCount - Số bài viết đã đăng trong 24 giờ qua
 */
```

---

## 3. Giao Thức WebSocket Message (Unified Protocol)

Hệ thống giao tiếp hai chiều thông qua WebSocket sử dụng một cấu trúc bản tin JSON thống nhất, được xác thực bằng schema Ajv.

### 3.1 Cấu Trúc Bản Tin Tổng Quát
```json
{
  "type": "HELLO | SESSION_EVENT | COMMAND | COMMAND_RESULT | HEARTBEAT",
  "timestamp": 1718706560000,
  "payload": {}
}
```

### 3.2 Schema Xác Thực Bản Tin (Ajv Schema)
```json
{
  "type": "object",
  "properties": {
    "type": { "type": "string", "enum": ["HELLO", "SESSION_EVENT", "COMMAND", "COMMAND_RESULT", "HEARTBEAT"] },
...
### 3.3 Các Loại Bản Tin & Payload Chi Tiết

#### 1. Bản tin `HELLO` (Client Handshake)
Client gửi bản tin này ngay khi thiết lập kết nối WebSocket để xác thực danh tính.
* **Payload Schema:**
```json
{
  "type": "object",
  "properties": {
    "accountId": { "type": "string", "pattern": "^[0-9a-zA-Z_.-]+$" },
    "nonce": { "type": "string", "minLength": 16 },
    "signature": { "type": "string", "pattern": "^[0-9a-fA-F]{64}$" }
  },
  "required": ["accountId", "nonce", "signature"],
  "additionalProperties": false
}
```
* **Luồng Xác Thực:** 
  $$\text{signature} = \text{HMAC-SHA256}(\text{accountId} + \text{nonce} + \text{timestamp}, \text{SharedSecret})$$

#### 2. Bản tin `COMMAND` (Server to Client)
Server gửi lệnh chỉ đạo AI Agent thực hiện hành động trên DOM của client.
* **Payload Schema:**
```json
{
  "type": "object",
  "properties": {
    ...
    "action": { "type": "string", "enum": ["CLICK", "TYPE", "WAIT", "NAVIGATE", "SCREENSHOT", "DONE", "ERROR"] },
...
#### 3. Bản tin `COMMAND_RESULT` (Client to Server)
Client gửi trả kết quả thực thi lệnh sau khi hoàn thành hành động.
* **Payload Schema:**
```json
{
  "type": "object",
  "properties": {
    ...
    "domSnapshot": { "$ref": "#/definitions/DOMSnapshot" }
  },
...
#### 4. Bản tin `SESSION_EVENT` (Client to Server)
Client cập nhật trạng thái hoạt động định kỳ hoặc đột xuất (sự kiện lỗi, phát hiện checkpoint).
* **Payload Schema:**
```json
{
  "type": "object",
  "properties": {
    "event": { "type": "string", "enum": ["INIT", "STARTED", "NAVIGATING", "OBSERVING", "ACTING", "COMPLETED", "CHECKPOINT_DETECTED", "ERROR"] },
...
## 4. Giao Thức Phân Mảnh Tệp Tin (Media Chunking Protocol)

Để truyền tải các file ảnh/video nặng từ Server xuống Extension mà không gây ngắt kết nối WebSocket do tải lượng quá lớn (Frame Overflow), hệ thống sử dụng giao thức phân nhỏ dữ liệu (Chunking).

### 4.1 Cấu Trúc Khởi Khởi Tạo Truyền Tải (`MEDIA_INIT`)
Trước khi truyền dữ liệu, Server gửi gói tin thông báo kích thước và tổng số chunk:
```json
{
  "type": "MEDIA_INIT",
  "timestamp": 1718706560000,
  "payload": {
    "mediaId": "media_uuid_v4",
    "mimeType": "video/mp4",
    "totalChunks": 12,
    "fileSize": 6291456
  }
...
### 4.2 Cấu Trúc Phân Đoạn Dữ Liệu (`MEDIA_CHUNK`)
Dữ liệu nhị phân được mã hóa Base64 và phân nhỏ thành các gói tin có kích thước tối đa 500KB:
```json
{
  "type": "MEDIA_CHUNK",
  "timestamp": 1718706560100,
  "payload": {
    "mediaId": "media_uuid_v4",
    "chunkIndex": 0,
    "data": "Base64StringDataHere...",
    "isFinal": false
  }
}
```

### 4.3 Quản Lý Bộ Đệm Và Ghép File (Media Reconstructor)
Trong môi trường Extension Client (Offscreen/Background), bộ đệm tích lũy media chunks được quản lý an toàn thông qua cấu trúc `Object.create(null)` để tránh Prototype Pollution:
```javascript
const mediaBuffer = Object.create(null);

function handleAttachMedia(msg) {
  const { mediaId, chunkIndex, totalChunks, data, isFinal, mimeType } = msg.payload;

  // 1. Kiểm tra RangeError giới hạn số chunk
  if (!Number.isInteger(totalChunks) || totalChunks <= 0 || totalChunks > 103) {
    throw new RangeError("Lỗi giới hạn totalChunks: Vượt ngưỡng 103 chunks cho phép");
  }

  if (!mediaBuffer[mediaId]) {
    mediaBuffer[mediaId] = {
  ...
  const buf = mediaBuffer[mediaId];
  buf.chunks[chunkIndex] = data;
  buf.received++;

  if (isFinal || buf.received === totalChunks) {
    // Kích hoạt giải mã phi chặn luồng (Non-blocking Base64 decoding)
    reconstructAndStoreMedia(mediaId);
  }
}
```
...
## 5. Chuẩn Hóa Cấu Trúc Dữ Liệu Cơ Sở Dữ Liệu (SQLite Schema)

Để đồng bộ và nhất quán việc lưu trữ thông tin thời gian trong cơ sở dữ liệu SQLite cục bộ, toàn bộ các trường thời gian phải được chuẩn hóa thành kiểu dữ liệu **Unix Epoch Milliseconds** (`INTEGER`).

### 5.1 Các Bảng Cơ Sở Dữ Liệu Chính

#### Bảng `accounts` (Quản lý tài khoản Facebook)
```sql
CREATE TABLE IF NOT EXISTS accounts (
  account_id TEXT PRIMARY KEY,          -- Định danh tài khoản (UID Facebook)
  account_name TEXT NOT NULL,           -- Tên hiển thị của tài khoản
  cookie_data TEXT,                     -- Dữ liệu cookies đã mã hóa
  proxy_url TEXT,                       -- SOCKS5/HTTP Proxy tương ứng
  status TEXT DEFAULT 'IDLE',           -- IDLE | RUNNING | CHECKPOINT | COOLDOWN
  health_score INTEGER DEFAULT 100,     -- Điểm sức khỏe tài khoản (0-100)
  created_at INTEGER NOT NULL,          -- Unix Epoch Milliseconds
  cooldown_until INTEGER DEFAULT 0      -- Thời gian hết thời gian chờ (Unix Epoch Milliseconds)
);
```

#### Bảng `campaigns` (Quản lý chiến dịch đăng bài)
```sql
CREATE TABLE IF NOT EXISTS campaigns (
  campaign_id TEXT PRIMARY KEY,         -- Định danh chiến dịch
  title TEXT NOT NULL,                  -- Tiêu đề chiến dịch
  post_content TEXT NOT NULL,           -- Nội dung bài viết mẫu (có Spintax)
  media_path TEXT,                      -- Đường dẫn file ảnh/video cục bộ
  status TEXT DEFAULT 'PENDING',        -- PENDING | RUNNING | COMPLETED | PAUSED
  created_at INTEGER NOT NULL           -- Unix Epoch Milliseconds
);
```

#### Bảng `posts_log` (Lịch sử tương tác đăng bài)
```sql
CREATE TABLE IF NOT EXISTS posts_log (
  ...
  group_name TEXT,
  status TEXT NOT NULL,                 -- SUCCESS | FAILED | SEARCH_FAILED
  post_url TEXT,                        -- URL bài đăng thành công
  error_message TEXT,                   -- Chi tiết lỗi nếu thất bại
  posted_at INTEGER NOT NULL,           -- Unix Epoch Milliseconds
...
---
Bản đặc tả kỹ thuật được phê duyệt thống nhất cho toàn hệ thống Hermes FacePost-Group.


---

### 🎯 [PRIMARY MODULE SPEC] Spec 08 - facepost_08_content_engine.md
